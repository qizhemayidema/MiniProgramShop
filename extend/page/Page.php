...
namespace page;
	class Page {
		 ...
		 * $page 当前页
		 */
		public function __construct($sdata, $listRows=15,$name,$page){
		...
		private function first(){
			...
			if($this->page==1)
				...
				$html.=" <a class='btn btn-primary-outline' href='javascript:void(0)' onClick='".$this->js_page_name."(1)'>首 页</a>";
		...
		private function prev(){
			...
			if($this->page==1)
				...
				$html.=" <a class='btn btn-primary-outline' href='javascript:void(0)' onClick='".$this->js_page_name.'('.($this->page-1).")'>上一页</a>";
		...
		private function pageList(){
			...
			for($i=$this->page-$inum;$i<=$this->page+$inum;$i++){
				...
				if($i == $this->page){
					$linkPage.=" <a class='current btn btn-blue'>{$i}</a>";
				}else{
					$linkPage.=" <a class='btn btn-primary-outline' href='javascript:void(0)' onClick='{$this->js_page_name}({$i})'>{$i}</a>";
				}
			}
		...
		private function next(){
			...
			if($this->page==$this->pageNum)
				...
				$html.=" <a class='btn btn-primary-outline' href='javascript:void(0)' onClick='{$this->js_page_name}(".($this->page+1).")'>下一页</a>";
		...
		private function last(){
			...
			if($this->page==$this->pageNum)
				...
				$html.=" <a class='btn btn-primary-outline' href='javascript:void(0)' onClick='{$this->js_page_name}({$this->pageNum})'>尾 页</a>";
